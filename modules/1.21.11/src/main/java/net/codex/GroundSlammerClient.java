...
// Fully compatible with Minecraft 1.21.x Fabric – no changes required.
public class GroundSlammerClient implements ClientModInitializer {
    ...
    private void handleLanding(ClientWorld world, PlayerEntity player, Entity entity, long nowMs, double prevVelY) {
        ...
        if (feetY <= spawnY + clientConfig.detection.spawnYEps || entity.isOnGround()) {
            ...
            if (onLeaves) {
                ...
                // Exception: mangrove, jungle and azalea leaves keep original texture colours
                if (blockUnder.isOf(Blocks.MANGROVE_LEAVES)
                        || blockUnder.isOf(Blocks.JUNGLE_LEAVES)
                        || blockUnder.isOf(Blocks.AZALEA_LEAVES)
                        || blockUnder.isOf(Blocks.FLOWERING_AZALEA_LEAVES)) {
    ...
    private void onGroundSlam(Entity entity, double downwardVelocity) {
        ...
        // Distance factor: 1 at 0, 0 at maxDistance (linear falloff)
        double factor = 1.0 - Math.sqrt(distance) / Math.sqrt(maxDistance);
        // Optional: steeper falloff
    ...
    private void spawnGroundSlam(ClientWorld world, PlayerEntity player,
        ...
        world.addParticleClient(new GroundSplashParticleEffect(
                slamType,
                sizeMultiplier,
                heightMultiplier,
                red, green, blue
    ...
    private ParticleType<LeafParticleEffect> getLeafParticle(BlockState state) {
        if (state.isOf(Blocks.MANGROVE_LEAVES)) {
            return ModParticles.LEAF_MANGROVE;
        } else if (state.isOf(Blocks.JUNGLE_LEAVES)) {
            return ModParticles.LEAF_JUNGLE;
        } else if (state.isOf(Blocks.AZALEA_LEAVES) || state.isOf(Blocks.FLOWERING_AZALEA_LEAVES)) {
            return ModParticles.LEAF_AZALEA;
        } else {
            return ModParticles.LEAF;
        }
    }
    ...
    private void spawnDropletBurst(ClientWorld world, double cx, double cy, double cz,
        ...
        ParticleType<SplashDropletParticleEffect> chosenType = rainingHere
                ? ModParticles.SPLASH_DROPLET_RAIN
                : ModParticles.SPLASH_DROPLET;
        ...
        for (int i = 0; i < count; i++) {
            ...
            world.addParticleClient(new SplashDropletParticleEffect(
                    chosenType,
                    amountMultiplier,
                    heightMultiplier,
                    red, green, blue
    ...
    public static void spawnLeafParticles(
        ...
        for (int i = 0; i < leafCount; i++) {
            ...
            world.addParticleClient(new LeafParticleEffect(
                    particleType,
    ...
    public static void spawnCherryBlossomParticles(
        ...
        for (int i = 0; i < particleCount; i++) {
            ...
            CherryBlossomParticleEffect effect = new CherryBlossomParticleEffect(
                    ModParticles.CHERRY_BLOSSOM,
                    r, g, b
            );

            world.addParticleClient(effect, spawnX, spawnY, spawnZ, vx, vy, vz);
        }
    }
